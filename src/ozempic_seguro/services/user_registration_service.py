...
class UserRegistrationService:
    ...
    def register(self, nome: str, username: str, senha: str, tipo: str) -> RegistrationResult:
        """
        Registra um novo usuário.

        Args:
            nome: Nome completo do usuário
            username: Nome de usuário (apenas números)
            senha: Senha (apenas números)
            tipo: Tipo de usuário (vendedor, repositor, administrador, tecnico)

        Returns:
            RegistrationResult com o resultado do registro
        """
        # Validar dados
        validation_errors = self._validate_registration_data(nome, username, senha, tipo)
        ...
        try:
            success, message = self._user_service.create_user(
                nome=nome, username=username, senha=senha, tipo=tipo
            )

            if success:
                logger.info(f"User registered successfully: {username}")
                return RegistrationResult(
                    success=True, message=f"Usuário '{nome}' cadastrado com sucesso!"
                )
            else:
    ...
    def _validate_registration_data(
        self, nome: str, username: str, senha: str, tipo: str
    ) -> List[str]:
        """
        ...
        if len(nome) > self.MAX_NAME_LENGTH:
    ...
    def validate_name(self, nome: str) -> bool:
        """Valida se o nome é válido"""
        return len(nome) <= self.MAX_NAME_LENGTH

    def validate_username(self, username: str) -> bool:
        """Valida se o username é válido"""
        return len(username) <= self.MAX_USERNAME_LENGTH and username.isdigit()

    def validate_password(self, senha: str) -> bool:
        """Valida se a senha é válida"""
        return (
            self.MIN_PASSWORD_LENGTH <= len(senha) <= self.MAX_PASSWORD_LENGTH and senha.isdigit()
        )
