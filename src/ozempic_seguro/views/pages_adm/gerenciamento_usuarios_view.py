...
class GerenciamentoUsuariosFrame(customtkinter.CTkFrame):
    ...
    def criar_tabela_usuarios(self):
        ...
        self.scrollable_frame = customtkinter.CTkScrollableFrame(
            self.tabela_frame, fg_color="white"
        )
    ...
    def carregar_dados(self):
        ...
        try:
            ...
            for idx, user in enumerate(usuarios):
                ...
                conteudo_frame = customtkinter.CTkFrame(linha_frame, fg_color="transparent")
                ...
                dados = [user.username, user.nome_completo, user.tipo_display]
                ...
                for col, texto in enumerate(dados):
                    ...
                    cell_frame = customtkinter.CTkFrame(conteudo_frame, fg_color="transparent")
    ...
    def criar_painel_direito(self):
        ...
        self.frame_detalhes = customtkinter.CTkFrame(self.painel_direito, fg_color="white")

        # Frame para os botões de ação
        self.frame_botoes = customtkinter.CTkFrame(self.painel_direito, fg_color="white")

        # Frame para mensagens (inicialmente vazio)
        self.frame_mensagem = customtkinter.CTkFrame(self.painel_direito, fg_color="white")
    ...
    def exibir_detalhes_usuario(self, user_id, username, nome_completo, tipo, ativo, data_criacao):
        ...
        for idx, (label, valor) in enumerate(campos):
            ...
            lbl_valor = customtkinter.CTkLabel(
                frame, text=valor, font=("Arial", 12), text_color="#333333"
            )
        ...
        btn_alterar_senha = ModernButton(
            ...
            if not is_tecnico
            else self.mostrar_aviso_tecnico,
        ...
        if is_tecnico:
            aviso_frame = customtkinter.CTkFrame(
                self.frame_detalhes, fg_color="#FFF3CD", corner_radius=8
            )
            aviso_frame.grid(
                row=len(campos) + 2, column=0, columnspan=2, padx=10, pady=20, sticky="ew"
            )
    ...
    def abrir_janela_alterar_senha(self):
        ...
        # Título
        lbl_titulo = customtkinter.CTkLabel(
            campos_frame, text="Alterar Senha", font=("Arial", 16, "bold")
        )
        ...
        self.entry_nova_senha.bind(
            "<Button-1>", lambda e: self.definir_campo_ativo(self.entry_nova_senha)
        )

        # Campo Confirmar Senha
        lbl_confirmar_senha = customtkinter.CTkLabel(
            campos_frame, text="Confirmar Senha:", anchor="w"
        )
        ...
        self.entry_confirmar_senha.bind(
            "<Button-1>", lambda e: self.definir_campo_ativo(self.entry_confirmar_senha)
        )

        # Rótulo para mensagens de erro/sucesso
        self.lbl_mensagem = customtkinter.CTkLabel(
            campos_frame, text="", text_color="red", wraplength=300, justify="left"
        )
        ...
        customtkinter.CTkLabel(
            teclado_frame, text="Teclado Numérico", font=("Arial", 14, "bold"), text_color="black"
        ).pack(pady=(10, 5))
    ...
    def mostrar_mensagem_erro(self, mensagem):
        ...
        # Ícone de aviso
        icone_aviso = customtkinter.CTkLabel(frame_principal, text="⚠️", font=("Arial", 24))
    ...
    def mostrar_mensagem_sucesso(self, mensagem):
        ...
        # Frame principal
        frame_principal = customtkinter.CTkFrame(
            janela_sucesso, fg_color="#dff0d8", corner_radius=10
        )
        frame_principal.pack(padx=20, pady=20, fill="both", expand=True)

        # Ícone de sucesso
        icone_sucesso = customtkinter.CTkLabel(frame_principal, text="✔️", font=("Arial", 24))
    ...
    def criar_botao_voltar(self):
        ...
        self.voltar_btn = VoltarButton(self, command=self.voltar_callback)
